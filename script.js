...
const game_processor = function () {
  ...
  const dice_is_rolled = dice_roll();
  const sound = new Audio('dice-142528.mp3');
  sound.play();

  for (let i = 1; i <= 6; i++) {
    if (dice_is_rolled === i) {
      dice_image.src = `dice-${i}.png`; // Dice image being displayed
      ...
      if (kaliya_ji) {
        current_rolls_1.innerHTML = i; // current throw begin displayed
        if (dice_is_rolled != 1) {
          player_1.innerHTML = Number(player_1.innerHTML) + dice_is_rolled; // throw points being accumulated cuz it is not 1
        }
        if (dice_is_rolled === 1) {
      ...
      if (kaliya_ji === false) {
        current_rolls_2.innerHTML = i; // current throw begin displayed
        console.log('ageye');
        if (dice_is_rolled != 1) {
          player_2.innerHTML = Number(player_2.innerHTML) + dice_is_rolled; // throw points being accumulated cuz it is not 1
        }
        if (dice_is_rolled === 1) {
